"""Окно месяца ленты бизнес-сигналов — правило показа (ADR 0001, п. 6).
...
class FeedWindow:
    ...
    `start` — первый открытый месяц. Верхней границы у окна нет намеренно: статья
    с датой в будущем (ошибка разбора даты) остаётся в ленте с пометкой «дата в
    будущем», а не пропадает из всех экранов до наступления своего месяца.
    """
    ...
    def is_open(self, period_month: str) -> bool:
        """Можно ли править статью с таким месяцем периода (строка «ГГГГ-ММ»)."""
        return period_month >= month_key(self.start)

    def sql(self, alias: str = "a") -> str:
        """SQL-условие окна для WHERE.

        Месяц подставляется литералом, а не параметром: его формирует этот модуль из
        объекта `date` (только цифры и дефис), поэтому инъекции здесь быть не может.
        ...
        return f"{expr} >= '{month_key(self.start)}'"
