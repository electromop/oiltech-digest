...
## 15. Следующий этап: контроль качества действий агента

После добавления оценки качества, регулярности и общего health-verdict главный
риск смещается с “найдёт ли агент источник” на “не будет ли агент делать
повторяющиеся или вредные действия”. Поэтому следующий слой развития — не новый
поиск, а контроль действий перед записью в систему.

### 15.1. Action policy перед записью в БД

Нужна единая функция проверки действия, например `validate_agent_action(...)`.
Она должна вызываться перед действиями:

- создание кандидата источника;
- постановка кандидата на оценку;
- повторная проверка кандидата;
- рекомендация изменения частоты источника;
- будущие полуавтоматические approve-like действия.

Проверки:

- URL/домен уже есть в текущих источниках;
- URL/домен уже есть среди кандидатов;
- домен находится в `rejected` или `muted` памяти;
- домен находится в cooldown после timeout, 502, anti-bot или временной ошибки;
- в текущем запуске агент уже пытался сделать такое же действие;
- за один запуск не создаётся слишком много кандидатов по одной теме;
- за один запуск не создаётся слишком много кандидатов с одного домена;
- действие не нарушает суточные бюджеты;
- действие требует подтверждения человека.

Результат policy должен быть структурным:

```json
{
  "allowed": false,
  "decision": "blocked",
  "reason": "Домен уже есть в каталоге источников",
  "checks": [
    {"name": "existing_source", "status": "blocked"},
    {"name": "cooldown", "status": "passed"}
  ]
}
```

### 15.2. Идемпотентность agent loop

Повторный запуск или ретрай не должен создавать пачку одинаковых кандидатов и
задач. Для этого нужны устойчивые ключи:

- `create_source_candidate:{normalized_url}:{topic}`;
- `evaluate_source_candidate:{candidate_id}:{article_limit}:{mode}`;
- `audit_existing_source:{source_id}:{date_bucket}`;
- `discover_topic:{topic}:{query_strategy}:{date_bucket}`.

Перед созданием новой записи или фоновой задачи агент должен проверять, не было ли
уже такого действия в текущем запуске или в заданном временном окне.

### 15.3. Agent decision log

Каждое действие агента должно сохранять не только результат, но и объяснение
проверок:

- что агент хотел сделать;
- какие входные данные использовал;
- какие policy-проверки прошёл;
- какие проверки заблокировали действие;
- итог: выполнено, пропущено, заблокировано, отправлено человеку;
- idempotency key;
- связь с `agent_run_id`.

Это нужно для UI и расследования проблем: оператор должен видеть не только “агент
ничего не сделал”, а конкретно почему он ничего не сделал.

### 15.4. UI для контроля действий

На странице агента нужен блок “Почему агент не сделал действие”. В нём показывать:

- заблокированные действия;
- причину блокировки;
- домен/тему/URL;
- policy-checks;
- кнопку “разрешить вручную” только там, где это безопасно.

Это позволит не гадать, почему кандидат не появился: дубль, cooldown, лимит,
ручной запрет или ошибка доступности.

### 15.5. Практический порядок реализации

1. Сделать DB-free модуль `source_discovery/action_policy.py`.
2. Подключить policy в `discover_sources` перед `upsert_source_candidate`.
3. Подключить policy в `source_discovery_loop` перед постановкой evaluation jobs.
4. Записывать policy result в `agent_actions`.
5. Вывести blocked/skipped действия на экран агента.
6. Добавить тесты на дубли, cooldown, rejected memory, лимиты и повтор в одном run.
