...
новый §11 — на `5c62f97` (2026-09-18).
...
- `request` — [`request_parser.parse_source()`](../oiltech_digest/ingestion/request_parser.py):
  ...
  `REQUEST_ARTICLE_LIMIT` (6) кандидатов и вставляет те, чьего `url` ещё нет в базе.
  Новизна держится на уникальности `articles.url`; поля `last_seen_*` и `last_listing_hash`
  пишутся только для диагностики — прежние «замораживатели» на них сняты. Как именно
  выбираются 6 кандидатов и почему это слабое место — §11.3;
...
### 11.3 Известные слабости парсера (на 18.09 не исправлены)

1. **Выбор кандидатов.** Ссылки сортируются по баллу, при равном — **по алфавиту
   адреса**; берутся первые `REQUEST_ARTICLE_LIMIT` = 6. Порядок страницы (свежее
   сверху) выбрасывается; дата из `article_date_selector` к кандидату прикрепляется, но в
   сортировке не участвует. Закреплённые пункты занимают места навсегда. Касается всех
   `request` и `playwright` источников.
2. **Ссылка без текста отбрасывается** (заголовок < 18 знаков) — даже при настроенном
   селекторе. Так свёрстаны карточки Белоруснефти, Б1, «Яков и Партнёры»: ссылка
   обёрнута вокруг картинки, заголовок рядом.
3. **Дата публикации из статьи не извлекается** на ЦДУ ТЭК, Сколтехе, ТеДо; у ТПУ её в
   статье нет вовсе — только в списке. Итог: `published_at` пуст, фильтр по возрасту не
   работает, старое ложится как новое.
4. **Браузер парсера ждёт 3,5 с** (`settle_ms`), Shell рисует список ~6 с.
5. **Mubadala и JPT R&D отбрасываются на шаге статьи** при верной ссылке (у JPT
   селектор находит 12 статей) — причина не разобрана.

Пункты 1–2 выведены из кода и подтверждены примерами, но не прогоном парсера —
проверить до правки.
